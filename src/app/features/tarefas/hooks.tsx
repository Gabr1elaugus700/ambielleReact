...
export function useTarefas() {
  ...
  return {
    ...
    create: async (data: TarefaInput) => {
      ...
      mutate((prev: Tarefa[] | undefined) => (prev ? [...prev, newTarefa] : [newTarefa]), false) // Otimista
      return newTarefa
    },
    update: async (id: number, data: Partial<TarefaInput>) => {
      const updatedTarefa = await updateTarefa(id, data)
      mutate((prev: Tarefa[] | undefined) => prev?.map(c => (c.id === id ? updatedTarefa : c)), false) // Otimista
      return updatedTarefa
    },
    remove: async (id: number) => {
      await deleteTarefa(id)
      mutate((prev: Tarefa[] | undefined) => prev?.filter(c => c.id !== id), false) // Otimista
    },
  }
}
